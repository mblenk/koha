package Koha::REST::V1::LLMProviders;
...
sub list {
    ...
    return try {
        return $c->render(
            status  => 200,
            openapi => $c->objects->search( Koha::LLMProviders->new )
        );
    } catch {
...
sub get {
    ...
    return try {
        my $provider = $c->objects->find(
            Koha::LLMProviders->new,
            $c->param('llm_provider_id')
        );
        return $c->render_resource_not_found("LLM provider")
            unless $provider;
...
sub add {
    ...
    return try {
        ...
        my $provider = Koha::LLMProvider->new_from_api($body);
        $provider->api_key( Koha::Encryption->new->encrypt_hex($api_key) ) if $api_key;
        $provider->store;
        $c->res->headers->location( $c->req->url->to_string . '/' . $provider->llm_provider_id );
        return $c->render(
            status  => 201,
